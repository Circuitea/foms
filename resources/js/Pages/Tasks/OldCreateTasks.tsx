...
type NewTask = Omit<Task, 'type' | 'priority' | 'creator' | 'due_date' | 'created_at' | 'updated_at' | 'status' | 'personnel'> & {
  due_date: Dayjs,
  personnel: number[],
};

/**
 * @deprecated Kept for reference purposes, will be deleted on release.
        ...
        <div className="sticky top-0 z-50 shadow border-b border-gray-200" style={{ backgroundColor: "#1B2560" }}>
          ...
          <TabsList>
            {[
              ...
              <TabsTrigger value={tab.id}><tab.icon className="w-4 h-4" /> {tab.label}</TabsTrigger>
            ))}
          </TabsList>
