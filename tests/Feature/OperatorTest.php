...
test('field operators can access dashboard', function () {
  $response = $this->actingAs($this->user)->get('/dashboard');

  $response->assertStatus(200);
});

test('field operators can access map', function () {
  $response = $this->actingAs($this->user)->get('/map');

  $response->assertStatus(200);
});

test('field operators can access my tasks', function () {
  $response = $this->actingAs($this->user)->get('/my-tasks');

  $response->assertStatus(200);
});

test('field operators can access tasks list', function () {
  $response = $this->actingAs($this->user)->get('/tasks');

  $response->assertStatus(200);
});

test('field operators can access new task form', function () {
